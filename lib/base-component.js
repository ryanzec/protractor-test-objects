...
var baseComponent = {
  ...
  /**
   * Returns the number of elements there are for the passed selector.
   *
   * @method getNumberOfElements
   *
   * @param {string} CSS selector
   *
   * @return {promise} A promise that will resolve to the number of  element the selector matches
   */
  getNumberOfElements: function(selector) {
    var defer = protractor.promise.defer();

    element.all(by.css(selector)).then(function(elements) {
      defer.fulfill(elements.length);
    }, function(error) {
      refer.reject(error);
    });
